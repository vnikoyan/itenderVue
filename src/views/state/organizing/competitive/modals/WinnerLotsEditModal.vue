          ...
          <table class="table m-0" style="border-collapse: collapse; border-spacing: 0; width: 100% !important;;">
            ...
            <tbody id="comparison-table" style="min-height: 5vh; max-height: 40vh!important; overflow: scroll">
              <template v-for="(lot, lotIndex) in participantGroup.won_lots">
                <tr :key="lotIndex">
                  <td>{{ lot.view_id }}</td>
                  <td>{{ lot.winner_lot_cpv_code }}{{ lot.procurement_plan.cpv_drop && '/'}}{{lot.procurement_plan.cpv_drop }}</td>
                  <td v-if="lot.is_from_outside">
                    <input
                      v-model="lot.winner_lot_cpv_name"
                      name="name"
                      type="text"
                      class="form-control"
                    >
                  ...
                  <td>
                    <input
                      v-if="currentOrganized.cpv_type !== '2'"
  ...
  export default {
    ...
    methods: {
      ...
      handleBeforeOpen({params}) {
        console.log(this.currentOrganized.cpv_type)
        this.participantGroup = params
        this.participantGroup.won_lots.forEach((lot) => {
          lot.winner_lot_cpv_code = lot.is_from_outside ? lot.procurement_plan.cpv_outside.code : lot.procurement_plan.cpv.code
          lot.winner_lot_cpv_name = lot.is_from_outside ? lot.procurement_plan.cpv_outside.name : lot.procurement_plan.cpv.name
          ...
          this.participantGroup.lots.push(lot);
        })
        console.log(this.participantGroup.lots)
      },
      save() {
        if(!this.$v.participantGroup.$invalid) {
          ...
          this.participantGroup.won_lots.forEach((lot) => {
            const updateData = {  
              winner_lot_cpv_name : lot.winner_lot_cpv_name,
