      ...
      <div class="row">
        <div class="col-6 steps">
          ...
          <ol>
            ...
            <li>Ընտրել մուտքագրված չափաբաժինների գնման ձևը և պայմանով լինել/չլինելը</li>
          </ol>
        </div>
        <div class="col-6">
          ...
          >
            <label class="text-dark" for="type">Գնման Ձև</label>
              <select
                ...
                class="form-control">
                <option :key="key" v-for="(type, key) in purchaseTypesForCompetetive" :value="key">{{ type }}</option>
              </select>
          </div>
        </div>
        <div class="col-6">
          ...
          <div v-if="+is_condition" class="col-10">
            <div class="form-check">
              <input :disabled="selectedIsConditionType !== false" v-model="condition_type" value="first_point" class="form-check-input" type="radio" name="conditionType" id="conditionType1">
              <label v-tooltip="selectedIsConditionType !== false ? 'մեկ ընթացակարգում չի կարող լինել տարբեր կետերի հիման վրա չափաբաժին':''" class="form-check-label" for="conditionType1">
                15-րդ հոդվածի 6-րդ մասի 1-ին կետի հիման վրա
              </label>
            </div>
            <div class="form-check">
              <input :disabled="selectedIsConditionType !== false" v-model="condition_type" value="second_point" class="form-check-input" type="radio" name="conditionType" id="conditionType2" checked>
              <label v-tooltip="selectedIsConditionType !== false ? 'մեկ ընթացակարգում չի կարող լինել տարբեր կետերի հիման վրա չափաբաժին':''" class="form-check-label" for="conditionType2">
                15-րդ հոդվածի 6-րդ մասի 2-ին կետի հիման վրա
              </label>
            </div>
          </div>
          <div class="form-check">
    ...
    methods: {
      ...
      beforeOpen(event) {
        if(event.params.length){
          ...
          this.condition_type = event.params[0].condition_type
          this.selectedType = event.params[0].type
          this.selectedIsCondition = event.params[0].is_condition
          this.selectedIsConditionType = event.params[0].condition_type
        }
      },
      removeRow(index){
          ...
          this.$client.post('parseRowsFromExel', {
            ...
            for(const rowString of rows) {
              ...
              if(columns.filter((col) =>col!=="").length !== rowModelKeys.length) {
                isDataInvalid = true
                console.log(rowModelKeys.length)
                console.log(columns)
                break
              }
