  ...
  >
    ...
    <div id="modal-body">
      ...
      <v-client-table ref="vtable" :columns="columns" v-model="contract.lots" :options="options">
        ...
        <div slot="active_orders_count" slot-scope="{row}">
          {{row.ordered}}
        </div>
          ...
          <div v-if="row.available" class="row m-0 p-0">
            <div class="col-12 p-0" v-tooltip="row.has_failed_order && 'Տվյալ չափաբաժնով կա չկայացած պատվեր'">
              <input
                ...
                v-positive-only.none-zero.integer
                type="text"
                min="1"
                class="form-control"
              >
...
export default {
  ...
  data () {
    return {
      ...
      isSuccess: true,
      lotsWithError: [],
      columns: ['index_number', 'cpv_name', 'specification', 'unit', 'purchase_schedule', 'unit_price', 'total_price', 'count', 'leftover', 'active_orders_count', 'order_input', 'order_price'],
      contract: {},
      lots: [],
      index: 0,
      focusedLotFields: {},
      ...
      options: {
        ...
        headings: {
          ...
          leftover: 'Մնացորդ',
          active_orders_count: 'Ընթացքի մեջ',
  ...
  methods: {
    ...
    beforeClose(){
      this.$router.push({ path: '/contractmanagement/contracts/' })
    },
    beforeOpen({params}) {
      this.resetComponent = params.resetComponent
      this.contract = params.contract
      if(this.contract.organize.type === 'competitive'){
        this.columns = ['index_number', 'cpv_name', 'specification', 'unit', 'unit_price', 'total_price', 'count', 'leftover', 'active_orders_count', 'order_input', 'order_price'];
      } else {
        this.columns = ['index_number', 'cpv_name', 'specification', 'unit', 'purchase_schedule', 'unit_price', 'total_price', 'count', 'leftover', 'active_orders_count', 'order_input', 'order_price'];
      }
    },
