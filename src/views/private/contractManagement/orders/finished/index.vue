              ...
              <div class="col-12 tenders-list p-0">
                  ...
                  >
                    <div slot="beforeTable" class="padding-20 pb-0 text-center text-md-left">
                      <div class="d-inline-block tab-block">
                        <ul class="nav nav-tabs align-items-end suggestions-tab-mobile" id="myTab" role="tablist">
                          ...
                          <li class="nav-item" role="presentation">
                            <a class="nav-link active" id="profile-tab" data-toggle="tab" href="#finished" @click.prevent="$router.push('/participant/contract-management/orders/finished')" role="tab" aria-controls="profile" aria-selected="false">
                              Ավարտված
                            </a>
                          </li>
                          <li class="nav-item" role="presentation">
                            <a class="nav-link" id="contact-tab" data-toggle="tab" href="#canceled" @click.prevent="$router.push('/participant/contract-management/orders/canceled')" role="tab" aria-controls="contact" aria-selected="false">
                              Չկատարված
                            </a>
                          </li>
                        </ul>
                    ...
                    <div slot="provider_name" class="d-flex justify-content-start align-items-center h-100" slot-scope="{row}">
                      <span class="w-100">{{row.contract.client.name}}</span>
                    </div>
                    ...
                    <div slot="docs" slot-scope="{row}">
                      <div class="row align-items-center justify-left m-0 p-0">
                        <div class='p-0 col-12 mb-2 text-primary text-left'>
                          <button
                          ...
                          >
                            ...
                            <span>{{isMobile ? 'Արձանագրություն' : 'Հանձնման ընդունման արձանագրություն'}}</span>
                          </button>
                        </div>
                        <div v-if="row.contract.organize_type !== 'itender'" class='p-0 col-12 mb-2 text-primary text-left'>
                          <button
                            class="btn btn-light d-flex align-items-center"
                            :disabled="!row.is_full"
                            @click.prevent="() => {
  ...
  export default {
    data() {
      return {
        ...
        columns: ['contract_code', 'provider_name', 'lots_count', 'lots', 'total_price', 'status', 'docs', 'action'],
        options: {
          requestAdapter (data) {
            return {
          ...
          headings: {
            ...
            provider_name: 'Պատվիրատուի անվանում',
    ...
    methods: {
      cancelOrder(order){
        this.$fire({
          ...
          title: "Զգուշացում",
          text: "Խնդրում ենք հաստատել կամ մերժել չեղարկումը",
          type: "warning",
          confirmButtonText: 'Հաստատել',
          cancelButtonText: 'Մերժել',
        }).then((result) => {
          if(result.value){
            this.$client.get(`contract-orders/cancel/${order.id}`).then(({data}) => {
              this.resetComponent()
            })
          }
        }
