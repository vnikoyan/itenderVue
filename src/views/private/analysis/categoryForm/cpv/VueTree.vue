<template>
  <div>
    ...
    <div v-if="node.name" @click="handleClick"
      class="mt-2" 
      :style="`margin-left: ${25 + (depth - 1) * 25}px;`">
      <div style="cursor: pointer;" v-if="node.children_count || loadRootOptions" aria-expanded="false">
        <div class="form-check d-inline-block mr-1" v-if="node.statistics_count">
          <label>
          <input @change="$emit('select', node)" 
              ...
              name="cpv"
              type="radio"
              :disabled="node.statistics_count === 0"
              class="">
        </label>
        </div>
        <svg class="mr-2 mb-1" v-if="isOpen" width="13" height="8" viewBox="0 0 13 8" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <svg class="mr-2 mb-1" v-else width="9" height="14" viewBox="0 0 9 14" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div v-else-if="!node.children_count" class="form-check p-0 d-flex" v-tooltip="statistics_count === 0 && 'Դեռևս տվյալներ չկան'">
        <label>
          <input @change="$emit('select', node)" 
              :id="node.id"
              :v-model="selectedOption"
              ...
              :disabled="node.statistics_count === 0"
              class="">
          <span :for="node.id" style="margin-left: 8px; cursor: pointer;" class="form-check-label"> <span class="node-code">{{ node.code }}</span> - {{ node.name }}</span>
        </label>
        <hr/>
      </div>
    </div>
    ...
    <transition name="dropdown" tag="div">
      <div v-show="(isOpen || loadRootOptions) && (node.children && node.children.length)">
        <VueTree v-for="child in node.children"
                ...
                @categorySelect="$emit('categorySelect', $event)"
  ...
  export default {
    ...
    methods: {
      lessThen4Numbers({code}){
        ...
        const codeSplittedCutted = codeSplitted.splice(1, 6)
        const result = codeSplittedCutted.filter(str => str !== '0').length >= 5
        return !result;
      },
      consoleLog(data){
        console.log(data)
      ...
      isAnyChecked() {
        ...
        if(_.intersectionBy(this.node.children, this.selectedOption, 'id').length) {
          this.isOpen = true
          this.$emit('open')
        }
      },
