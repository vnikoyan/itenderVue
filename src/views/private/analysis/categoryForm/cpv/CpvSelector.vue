<template>
  <div @click="handleClickOutside" class="row w-100 m-0 p-0">
    <div class="col-12 cpvs-list" :class="!isMobile && 'h-100'">
      <ul class="nav nav-justified cpv_selector-nav" :class="isMobile ? 'justify-content-between' : 'justify-content-center '" role="tablist">
      ...
      <div id="cpv-tree-containere" class="tab-content pt-3" :style="!isMobile && 'height: calc(100% - 22px);'">
        <div class="d-flex justify-content-center">
          <div class="mb-2 position-relative cpv-search-block">
            <input 
              @input="handleSearch"
              id="search"
              :value="searchQuery" 
        ...
        <div v-show="!isMobile || !showSelectedCpvs" class="w-100 little-scroll pl-0" :style="!isMobile && 'height: 524px; overflow: auto'">
          <vue-tree 
...
<script>
  ...
  export default {
    ...
    watch: {
      selectedOption: function(updatedOption) {
        this.$emit('selectCpv', updatedOption)
      }
    },
    mounted() {
      this.selectedOption = this.$attrs.selectedCpvs;
